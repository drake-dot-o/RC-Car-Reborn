...
local Input = {
   ...
   Boost    = keybinds:fromVanilla("key.sneak"), -- Use vanilla shift key for boost
}
...
local RC = {
   ...
   a_boost = 2,            -- Boost Speed Multiplier
   a_f = 0.8,                -- Friction
...
events.TICK:register(function ()
   ...
   -- Handle boost input
   local boost_active = Input.Boost:isPressed() and RC.engine
   if boost_active ~= RC.boost then
      pings.GNRCcarBoost(boost_active)
   end
   
   RC.lstr = RC.str
   local base_speed = math.lerp(RC.a_s, RC.a_sf, RC.e_a)
   local current_speed = base_speed
   if RC.boost then
      current_speed = base_speed * RC.a_boost -- Apply boost multiplier
   end
   RC.et = RC.et * 0.7 + current_speed * RC.ctrl.y * 0.4
...
---Returns true if boost is currently active
---@return boolean
function API:isBoostActive()
   return RC.boost
end
...
function pings.GNRCcarBoost(boost_active)
   RC.boost = boost_active
   API.ON_BOOST:invoke(boost_active)
end
...
events.POST_WORLD_RENDER:register(function (dt)
   ...
   if Camera.transition < 0.01 then
      ...
      local boost_fov_multiplier = RC.boost and 1.3 or 1 -- Extra FOV when boosting
      renderer:setFOV(math.lerp(1,math.lerp(1,RC.a_sf_fov_mul * boost_fov_multiplier,RC.e_a),Camera.transition))
   end
end)
